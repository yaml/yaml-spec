...
A brutally hacked version of EBNF that works around FO's inability to
automatically determine the width of table columns. It merges the lhs, ::= and
rhs columns into a single one, allowing the production to look the way we want
in the YAML spec:

    c-printable ::= #x9 | #xA | #xD
                  | [#x20-#x7E] | #x85
                  | [#xA0-#xD7FF]
                  | [#xE000-#xFFFD]
                  | [#x10000-#x10FFFF]
...
Note that the '::=' is NOT aligned, unlike the original EBNF layout. Also,
to properly indent multiple production lines, it is necessary to add the
&nbsp; entities somehow. This is done in a pre-processing XSLT script; it
really should be done here instead.

Finally, the last column (annotation) is removed altogether. This allows us
to assign 95% of the page width to the production itself.

The original version header was:

     ********************************************************************
     $Id: ebnf_fo.xsl,v 1.1 2004/12/03 19:09:48 oren Exp $
     ********************************************************************
...
<xsl:template match="production">
  ...
  <fo:table-row>
    ...
    <fo:table-cell>
      <fo:block text-align="start" font-family="{$monospace.font.family}">
        <xsl:choose>
          <xsl:when test="$recap">
